Feature: TTMT List integration test
...
Scenario: create, update and clean list
    ...
    When method post
    Then status 200
    And match response == { type: 'ocap', ocapType: 'ListEditFacet', url: '#notnull' }
    ...
    When method get
    Then status 200
    And match response == { type: 'ListEditFacet', view_facet: '#notnull', contents: [] }
    ...
    When method put
    Then status 200
    And match response == { type: 'ListEditFacet', view_facet: '#notnull', contents: [{ type: 'ocap', ocapType: 'MediaViewFacet', url: '#(responseUrl)' }] }
